...
def loss_function(y_conv, label_batches):
    ...
    for i in range(num):
        ...
        for j in range(i+1, num):
            b2 = y_conv[:, j]
            l2_dis = tf.reduce_sum(tf.square(b1-b2))
            norm = alpha * (tf.reduce_sum(tf.abs(tf.subtract(tf.abs(b1), 1.0))) + tf.reduce_sum(tf.abs(tf.subtract(tf.abs(b2), 1.0))))
...
def main():
    ...
    train_step = tf.train.MomentumOptimizer(0.001, momentum).minimize(loss, global_step=global_step)
    ...
    with tf.Session(config=config) as sess:
        ...
        writer = tf.summary.FileWriter("logs0/", sess.graph)
